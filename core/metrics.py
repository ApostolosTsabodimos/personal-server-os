...
def _ensure_schema(db_path):
    """Create/migrate metrics table. Opens its OWN raw connection."""
    import sqlite3
    ...
    try:
        ...
        for col_name, col_def in METRICS_REQUIRED_COLS.items():
            if col_name not in existing:
                try:
                    conn.execute(f"ALTER TABLE metrics ADD COLUMN {col_name} {col_def}")
                except Exception as e:
                    pass  # metric collection best-effort — individual failures don't stop the loop
        conn.commit()
    finally:
        conn.close()
...
class MetricsCollector:
    ...
    def start_daemon(self):
        """Start collecting in a background daemon thread."""
        import threading
        def _loop():
            while True:
                try:
...
def cmd_show(_args):
    """pso metrics show — display latest value of every collected metric"""
    store = MetricsStore()
    ...
    with store.db._get_connection() as conn:
        rows = conn.execute("""
            ...
            WHERE collected_at > ?
            GROUP BY metric, service_id
            ORDER BY metric, service_id
        """, (since,)).fetchall()

    if not rows:
        print("\n  No metrics in the last 10 minutes.")
        print("  Run 'pso metrics collect' first.\n")
        return

    print(f"\n  {BOLD}Latest Metrics{RESET}  (last 10 min)")
    print("  " + "─" * 60)
    print(f"  {'Metric':<32} {'Service':<16} {'Value':>10}  {'When'}")
    print("  " + "─" * 60)

    prev_metric = None
    for r in rows:
        metric  = r["metric"] if hasattr(r, "__getitem__") else r[0]
        svc     = r["service_id"] or "host"
        val     = r["value"] if hasattr(r, "__getitem__") else r[2]
        ts      = (r["ts"] if hasattr(r, "__getitem__") else r[3] or "")[:16].replace("T", " ")

        # Format value nicely based on metric name
        ...
        # Dim separator between different metrics
        if metric != prev_metric and prev_metric is not None:
            print(f"  {'·'*58}")
        prev_metric = metric

        svc_display = f"{DIM}{svc}{RESET}"
        print(f"  {metric:<32} {svc_display:<25} {val_str:>10}  {DIM}{ts}{RESET}")

    print()
    print(f"  {DIM}pso metrics query <metric>         — full history")
    print(f"  pso metrics export                 — Prometheus format{RESET}\n")
